...
- 감사보고서 PDF 업로드 후 재무제표 자동 분석 기능 추가
- PDF에서 추출한 재무데이터를 기존 재무분석 엔진과 연동
- 메인 화면에서 **기업 검색 없이 감사보고서 PDF를 바로 업로드**할 수 있는 기능 추가
- 분석 출처 표시 기능 추가 (DART / 감사보고서 PDF)
- 데이터 개수에 따라 분석 기간(2개년 / 3개년) 자동 표시
- PDF 보고서 제목 및 Footer 개선
- PDF 업로드 진행 상태 UI 개선

### 🔧 개선 사항

- DART 분석과 PDF 분석 간 상태 충돌 문제 개선
- 다른 기업 검색 시 PDF 분석 상태 초기화
- PDF 분석 UX 개선
- PDF 직접 업로드와 DART 검색 분석을 모두 지원하도록 사용자 흐름 개선